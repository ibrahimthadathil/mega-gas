...
import { getAllUnloadDetails } from "@/services/client_api-Service/user/unload/unload_api";
...
const page = () => {
  ...
  const columns = useMemo(() => {
    return [
      ...
      {
        header: "Warehouse",
        render: (row: PlantLoadUnloadView) => <span>{row.warehouse.name}</span>,
      },
      {
        header: "Total QTY",
      ...
      },
      {
        ...
        render: (row: PlantLoadUnloadView) => (
          <Button
            variant="ghost"
            onClick={() => alert(row.plant_load_register_id)}
          >
        ...
        render: (row: PlantLoadUnloadView) =>
          role == "admin" ? (
            <AlertModal
              ...
              style=" p-2"
              varient="ghost"
              action={() => alert(row.plant_load_register_id)}
            />
          ) : (
            <>
