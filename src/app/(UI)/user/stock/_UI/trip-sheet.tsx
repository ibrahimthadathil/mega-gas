...
export default function TripSheet({ loadSlipId }: { loadSlipId: string }) {
  ...
  const isEmptyQuantityValid = useMemo(() => {
    ...
    const totalQuantity = unloadRecord[0].line_items.reduce((sum: number, lineItem: any) => {
      return sum + (lineItem.qty || 0);
    }, 0);

    // Total empty quantity from trip load records
    const totalEmptyQty = tripLoadRecords.reduce((sum, record) => sum + record.emptyQuantity, 0);
  ...
  const onSubmit = async (data: TripFormData) => {
    ...
    if (!isFormValid) {
      if (!isFullyAllocated) {
        ...
        const requiredEmpty = unloadRecord?.[0]?.line_items?.reduce(
          (sum: number, item: any) => sum + (item.qty || 0), 
          0
        ) || 0;
        toast.error(`Total empty quantity must equal ${requiredEmpty}. Current: ${totalEmpty}`);
      }
      return;
    }
    ...
    <div className="w-full max-w-2xl mx-auto space-y-6">
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
        ...
        <Card className="p-6">
          ...
          {/* Allocation Status Alert */}
          {(!isFullyAllocated || !isEmptyQuantityValid) && tripLoadRecords.length > 0 && (
            <Alert className="mb-4 border-orange-500 bg-orange-50 dark:bg-orange-950">
              <AlertCircle className="h-4 w-4 text-orange-600" />
              <AlertDescription className="text-orange-800 dark:text-orange-200">
                <div className="space-y-1">
                  ...
                  {!isEmptyQuantityValid && (
                    <p>• Total empty quantity must equal total full quantity ({unloadRecord?.[0]?.line_items?.reduce((sum: number, item: any) => sum + (item.qty || 0), 0) || 0})</p>
                  )}
                </div>
              </AlertDescription>
            </Alert>
            ...
            <>
              ...
              <div className="flex gap-6 text-sm bg-muted/30 rounded p-3">
                ...
                <div>
                  <p className="text-muted-foreground">Total Empty Returned:</p>
                  <div className="flex items-center gap-2">
                    <p className={`font-semibold text-lg ${isEmptyQuantityValid ? 'text-green-600' : 'text-orange-600'}`}>
                      {totalEmpty}
                    </p>
                    <span className="text-xs text-muted-foreground">
                      / {unloadRecord?.[0]?.line_items?.reduce((sum: number, item: any) => sum + (item.qty || 0), 0) || 0}
                    </span>
                    {isEmptyQuantityValid ? (
...
//       // Check if edit mode
//       const isEdit = record.is_unloaded && record.unload_details?.length > 0;
...
//       setValue("helpers",  isEdit ? record.unloading_staff.map((e)=>e.id):delivery_boys || []);

//       // If edit mode, populate trip load records from unload_details
//       if (isEdit) {
//         const editRecords: TripLoadRecord[] = record.unload_details.map((detail: any) => ({
...
//           plant_load_line_item_id: record?.line_items.find(
//             (item: any) => item.product_id === detail.product.id)?.plant_load_line_item_id || "",
//           return_product_id: detail.return_product?.id || null,
//           return_warehouse_id: detail.to_return_warehouse?.id || null,
//         }));
//         setValue("tripLoadRecords", editRecords);
//       }
//     }
//   }, [unloadRecord, setValue, delivery_boys]);
...
//         ? await updateUnloadSlip(data) 
//         : await unloadSlip(data);
        
//       if (result.success) {
//         toast.success(isEditMode ? "Slip updated" : "Slip created");
//         router.push("/user/stock/load-slip");
//       }
//     } catch (error) {
//       toast.error(`Error in unload ${isEditMode ? 'update' : 'submission'}`);
//     } finally {
//       setSubmit(false);
//     }
//   };
...
//             You are editing an existing trip sheet
//           </AlertDescription>
//         </Alert>
//       )}
