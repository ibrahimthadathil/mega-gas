...
export default function CurrencyDenominationsSection({
  ...
  const totalCurrency = denominationDetails.reduce((sum, item) => sum + item.amount, 0)
  ...
  return (
    <div className="space-y-3">
      <h2 className="text-lg font-semibold text-foreground">Cash Denominations</h2>

      <div className="overflow-x-auto rounded-lg border border-border">
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-border bg-muted">
              <th className="px-3 py-2 text-left font-medium text-xs md:px-4 md:py-3">Currency</th>
              <th className="px-2 py-2 text-center font-medium text-xs md:px-4 md:py-3">Qty</th>
              <th className="px-3 py-2 text-right font-medium text-xs md:px-4 md:py-3">Amount</th>
            </tr>
          </thead>
          <tbody>
            {denominationDetails.map((item) => (
              <tr key={item.denom} className="border-b border-border hover:bg-muted/50">
                <td className="px-3 py-2 font-medium md:px-4 md:py-3">₹{item.denom}</td>
                <td className="px-2 py-2 md:px-4 md:py-3">
                  <div className="flex items-center gap-1 md:gap-2 justify-center">
                    ...
                    <input
                      type="number"
                      value={item.count}
                      onChange={(e) => handleChange(item.denom, Number.parseInt(e.target.value) || 0)}
                ...
                <td className="px-3 py-2 text-right font-semibold md:px-4 md:py-3">₹{item.amount.toLocaleString()}</td>
              </tr>
            ))}
          </tbody>
        </table>
      ...
      <Card className={`p-4 ${mismatch ? "bg-red-50 border-red-200" : "bg-primary/10"}`}>
        <div className="space-y-2">
          <div className="flex justify-between items-center">
            <span className="text-sm text-muted-foreground">Total Cash</span>
            <span className={`text-2xl font-bold ${mismatch ? "text-red-600" : "text-primary"}`}>
              ₹{totalCurrency.toLocaleString()}
            </span>
          </div>
          <div className="flex justify-between items-center pt-2 border-t border-border">
            <span className="text-sm text-muted-foreground">Expected (Net Sales)</span>
            <span className="text-sm font-medium">₹{netSales.toLocaleString()}</span>
          </div>
          {mismatch && (
            <div className="flex justify-between items-center pt-2 border-t border-red-200">
              <span className="text-sm font-medium text-red-600">Mismatch</span>
              <span className={`text-lg font-bold ${mismatchAmount > 0 ? "text-red-600" : "text-red-600"}`}>
                {mismatchAmount > 0 ? "+" : ""}₹{mismatchAmount.toLocaleString()}
              </span>
            </div>
          )}
        </div>
