...
export default function DeliveryPartnerSection({
  ...
  const handleAddDeliveryBoy = (deliveryBoyId: string) => {
    ...
    if (deliveryBoy && !selectedDeliveryBoys.some((db) => db.id === deliveryBoyId)) {
  ...
  return (
    <Card className="p-4">
      <div className="space-y-3">
        <h2 className="text-lg font-semibold text-foreground">Delivery Boys</h2>
          ...
          <div className="pt-2 border-t">
            ...
            <div className="flex flex-wrap gap-2">
              {selectedDeliveryBoys.map((deliveryBoy) => (
                ...
                >
                  <span className="text-sm font-medium">{deliveryBoy.name}</span>
                  <Button
                    size="sm"
                    variant="ghost"
