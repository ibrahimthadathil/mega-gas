...
export function StockDashboard({ 
  ...
  const groupedTransactions = useMemo(() => {
    ...
    const sorted = [...transactions].sort(
      (a, b) => new Date(a.transaction_date).getTime() - new Date(b.transaction_date).getTime() || a.idx - b.idx
    );
                    ...
                    <Fragment key={date}>
                      ...
                      {transactionsForDate.map((t) => (
                        <TableRow key={t.idx}>
                          ...
                          <TableCell className="font-medium">{t.product_name}</TableCell>
                          <TableCell>{t.counter_warehouse_name}</TableCell>
                          <TableCell className={cn("text-right font-mono", Number(t.qty_change) < 0 && "text-destructive")}>
